...
Este skill actualiza las herramientas de desarrollo del proyecto a la ultima version disponible.
Para compatibilidad con Codex, tambien debe revisar `.agents/`, `.codex/`, `AGENTS.md` y `CODEX.md`.
...
### Paso 3: Actualizar toolbox y capa Codex

Actualiza de forma aditiva estas rutas del proyecto actual:

```bash
cp -r [RUTA_REPO_SF]/saas-factory/.claude/. .claude/
cp -r [RUTA_REPO_SF]/saas-factory/.agents/. .agents/
cp -r [RUTA_REPO_SF]/saas-factory/.codex/. .codex/
```

Si `AGENTS.md` no existe, copialo desde el template.
Si `CODEX.md` no existe, copialo desde el template.

NO reemplaces `CLAUDE.md` automaticamente.
...
- .agents/skills/ (adaptadores para Codex)
- .codex/ (configuracion de proyecto para Codex)
- AGENTS.md y CODEX.md (si faltaban)
...
- Solo actualiza la "toolbox" de desarrollo y la capa de compatibilidad con Codex
- Revisa `CODEX_COMPATIBILITY.md` en este directorio cuando el objetivo sea llevar un proyecto viejo a Codex
